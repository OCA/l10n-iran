...
class BaseModelExtend(models.AbstractModel):
    ...
    def _read_group_format_result(self, data, annotated_groupbys, groupby, domain):
                    ...
                    try:
                        if ftype == 'datetime':
                            label = jd.date.fromgregorian(
                                day=value.day, month=value.month, year=value.year, locale='fa_IR').strftime(display_format_changer(gb['display_format']))
                        else:
                            label = jd.date.fromgregorian(
                                day=value.day, month=value.month, year=value.year, locale='fa_IR').strftime(display_format_changer(gb['display_format']))
                    except Exception as e:
                        _logger.warning('display_format_changer not found %s',gb['display_format']) 
                        if ftype == 'datetime':
        ...
        del data['id']
        return data
        # return super(BaseModelExtend, self)._read_group_format_result(data, annotated_groupbys, groupby, domain)
